---
output:
  github_document:
    html_preview: false
---

<!-- README.md is generated from README.Rmd. Please edit that file -->
<!-- build with rmarkdown::render("README.Rmd") -->

```{r setup, include = FALSE}
library(rvest)

knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%"
)
```
# [infx](https://nbenn.github.io/infx)

[![lifecycle](https://img.shields.io/badge/lifecycle-experimental-orange.svg)](https://img.shields.io/badge/lifecycle-experimental-orange.svg)
[![Travis-CI Build Status](https://travis-ci.org/nbenn/infx.svg?branch=master)](https://travis-ci.org/nbenn/infx)
[![AppVeyor Build Status](https://ci.appveyor.com/api/projects/status/github/nbenn/infx?branch=master&svg=true)](https://ci.appveyor.com/project/nbenn/infx)
[![Coverage status](https://codecov.io/gh/nbenn/infx/branch/master/graph/badge.svg)](https://codecov.io/github/nbenn/infx?branch=master)

Access to [InfectX](http://www.infectx.ch)/[TargetInfectX](https://www.targetinfectx.ch) screening data from R. A browser-based view of the data is available [here](http://www.infectx.ch/databrowser).

## Installation

You can install infx from github with:

```{r gh-installation, eval = FALSE}
# install.packages("devtools")
devtools::install_github("nbenn/infx")
```

## OpenBIS API

Documentation to the full JSON-RPC API can be accessed [here](https://wiki-bsse.ethz.ch/display/openBISDoc1304/openBIS+JSON+API). The available methods are grouped as:

```{r openbis-api, echo = FALSE, results = "asis", tidy = FALSE}
skip <- c("VisibleToUser", "OnBehalfOfUser", "^add", "^create", "^delete",
          "^remove", "^update", "put", "Metaproject", "Attachments",
          "SpacesWithProjects", "Vocabularies", "WebAppSettings", "Query",
          "Queries", "Aggregation", "TableReport", "RoleSet",
          "getSessionToken", "shuffle", "listAllShares", "getValidationScript",
          "getPathToDataSet", "getFileFromSessionWorkspace", "getFeatureList",
          "logoutScreening", "tryLoginScreening", "ImageTransformerFactory",
          "loadImages$", "loadPhysicalThumbnails$", "loadThumbnailImages$")
done <- c("tryToAuthenticateForAllServices", "logout$", "isSessionActive",
          "listProjects",
          "listExperiments", "listExperimentTypes",
          "listSamplesForExperiment", "getPlateSample", "getWellSample",
          "listSampleTypes",
          "listDataSets", "getDataSetMetaData", "listDataSetTypes",
          "getDatasetIdentifiers",
          "listDataStores",
          "listPlates", "listPlateWells",
          "getMaterialByCodes",
          "getDataStoreBaseURLs", "getDefaultPutDataStoreBaseURL",
          "tryGetDataStoreBaseURL",
          "listFilesForDataSet",
          "getDownloadUrlForFileForDataSet",
          "getDownloadUrlForFileForDataSetWithTimeout",
          "getExperimentImageMetadata", "getPlateMetadataList",
          "listImageDatasets", "listRawImageDatasets",
          "listSegmentationImageDatasets", "listImageMetadata",
          "listAvailableImageRepresentationFormats",
          "listPlateMaterialMapping",
          "listImageReferences", "listPlateImageReferences",
          "listFeatureVectorDatasets",
          "listAvailableFeatures", "listAvailableFeatureCodes",
          "loadFeatures", "loadFeaturesForDatasetWellReferences",
          "loadImagesBase64", "loadThumbnailImagesBase64",
          "loadPhysicalThumbnailsBase64", "^search.+s$")

api <- c("generic/shared/api/v1/IGeneralInformationService.html",
         "generic/shared/api/v1/IGeneralInformationChangingService.html",
         "plugin/query/shared/api/v1/IQueryApiServer.html",
         "generic/shared/api/v1/IWebInformationService.html",
         "dss/generic/shared/api/v1/IDssServiceRpcGeneric.html",
         "plugin/screening/shared/api/v1/IScreeningApiServer.html",
         "dss/screening/shared/api/v1/IDssServiceRpcScreening.html")

out <- sapply(api, function(url) {
  url <- paste("http://svnsis.ethz.ch/doc/openbis/13.04.0/ch/systemsx/cisd",
               "openbis", url, sep = "/")

  res <- html_nodes(read_html(url),
                    xpath = "/html/body/div[4]/div[2]/ul/li/ul[2]/li/table")

  res <- apply(html_table(res)[[1]], 1, function(x) {
    retr <- x[1]
    x <- x[2]
    name <- sub("\\($", "", regmatches(x, regexpr("^.+?\\(", x)))
    args <- sub("^\\(", "",
                sub("\\)$", "", regmatches(x, regexpr("\\(.+?\\)", x))))
    desc <- sub("\\)\n", "", regmatches(x, regexpr("\\)\n.+$", x)))

    found <- any(sapply(done, grepl, name))

    if (length(desc) && grepl("^Deprecated", desc))
      st <- TRUE
    else
      st <- any(sapply(skip, grepl, name))

    paste0("    + [", if (found) "x" else " ", "] ",
           if (st) "~~", name, if (st) "~~", ":\n        - description: ",
           gsub("\n", " ", desc), "\n        - arguments: ",
           paste(gsub(",\n +", ", ", args), collapse = ", "),
           "\n        - returntype: ", retr)
  })

  paste0("* [", sub("\\.html$", "", basename(url)), "](", url, ")\n",
         paste(res, collapse = "\n"))
})
cat(paste(gsub("<", "&lt;", gsub(">", "&gt;", out)), collapse = "\n"))
```
