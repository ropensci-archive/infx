...
The `json_vec` class wraps around a list of `json_class` and it serves to bring the common sub-class of all child objects to the surface for using method dispatch on. Instantiation of `json_vec` objects is possible in several ways: using the `json_vec()` constructor, by coercing a list structure with `as_json_vec()` or by combining several `json_class` objects using `base::c()`. The reverse action can be performed, using `base::as.list()`. 
