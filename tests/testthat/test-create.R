...
test_that("create PlateIdentifier objects", {
  ...
  expect_true(has_subclass(create_plate_id("foo", "bar"),
              "PlateIdentifier"))
  ...
  expect_true(has_subclass(id, "PlateIdentifier"))
...
test_that("create DatasetIdentifier objects", {
  ...
  img_ds <- list_img_datasets(tok, "KB01-2H", type = "segmentation",
                              most_recent = FALSE)
  expect_is(ds_id_1 <- create_dataset_id(tok, img_ds[[1]]), "json_vec")
  expect_true(has_subclass(ds_id_1[[1]], "DatasetIdentifier"))
  expect_is(ds_id <- create_dataset_id(tok, img_ds), "json_vec")
  expect_true(all(sapply(ds_id, has_subclass, "DatasetIdentifier")))
